import React from 'react';
    import Icon from '../components/Icon';
    import { IconComponent } from '../types';
    
    const MiniFridge: IconComponent = (props) => (
        <Icon viewBox="0 0 24 24" fill="undefined" {...props}>
            <g fill="currentColor"><path d="M6.78 12.73v2.94" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" /><path d="M3.85 1h16.13c.81 0 1.47.66 1.47 1.47v19.07c0 .81-.66 1.47-1.47 1.47H3.85c-.81 0-1.47-.66-1.47-1.47V2.47c0-.81.66-1.47 1.47-1.47z" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" /><path d="M2.38 8.33h19.07" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" /></g>
        </Icon>
    );
    
    export default MiniFridge;
    